...
- 检查并安装 Lynis
- 系统更新与自动安全更新
- 长时间软件更新阶段输出保活提示，降低远程 SSH 因长时间无输出而中断的风险
- 安装 `libpam-tmpdir`
- SSH 安全基线配置
- `sshd_config` 校验与回滚
- `rsyslog` 与日志权限配置
...
- `pwquality` 密码复杂度策略
- 登录 Banner 与 `su` 限制
- 系统加固仅支持 Debian / Ubuntu，其他发行版会在入口处直接拦截
...
- 远程 SSH 执行系统加固时，仍建议放在 `tmux` / `screen` 中运行，以降低链路抖动导致的中断影响
- 如果当前系统不是 Debian / Ubuntu，请不要使用本脚本的“系统加固”菜单
